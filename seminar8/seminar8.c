...
void afisareMail(Mail mail) {
	printf("Mail cu textul: %s si gradul de prioritate: %d\n", mail.text, mail.prioritate);
}
...
void main() {
	...
	heap.vector[0] = initializareMail("mail1", 7);
	heap.vector[1] = initializareMail("mail2", 9);
	heap.vector[2] = initializareMail("mail3", 3);
	heap.vector[3] = initializareMail("mail4", 8);
	heap.vector[4] = initializareMail("mail5", 6);
	...
	printf("\n\nInserare:\n");
	inserareMailInHeap(&heap, initializareMail("salll", 6));
	inserareMailInHeap(&heap, initializareMail("salut2", 10));
	inserareMailInHeap(&heap, initializareMail("Alo", 4));
	afisareHeap(heap);
