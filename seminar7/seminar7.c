...
void inserareGalerieInTabela(HashTable hash, Galerie galerie) {
	if (hash.dim > 0) {
		...
		else inserareLaFinal(&(hash.vector[pozitie]), galerie);
	}
}

void afisareGalerie(Galerie g) {
	printf("%d.Intrarea la galeria %s costa la %.2f \n", g.cod, g.nume, g.pretIntrare);
...
void main() {
	...
	Galerie g5 = initGalerie(5, "Geologie", 15);
	Galerie g6 = initGalerie(6, "Istorie", 17);

	HashTable tabela = initHash(6);
